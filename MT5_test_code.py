...
ID_String = "SetYourID"
PW_String = "SetYourPassword"

print("backend:", matplotlib.get_backend())
pio.renderers.default='svg' #Spyderに表示される svg,browser,png
...
if mt5.initialize( login=ID_String, server="XMTrading-MT5 2", password=PW_String ) == False:
...
df_all              = prepare_df_for_plot   ( 'USDJPY', tFm, bars_all )
#fig, ax             = create_fig_test       ( df_all, tFm, bars_plt, xrot, xsiz_mfp )
#mpf.show()
fig, ax             = create_fig            ( df_all, tFm, bars_plt, xrot, xsiz )
mpf.show()

#print( mt5.terminal_info()) # 接続状態、サーバ名、取引口座に関するデータを表示する
#print( mt5.version() ) # MetaTrader 5バージョンについてのデータを表示する
mt5.shutdown() # MetaTrader 5ターミナルへの接続をシャットダウンする
